...
class RecipeQueryModel: NSObject{
    ...
    func downloadItems(rCategory: Int, rLikeNum: Int, order: String, keyword: String, startIndex: Int, count: Int){
        
        let urlAddr = "?rCategory=\(rCategory)&rLikeNum=\(rLikeNum)&order=\(order)&keyword=\(keyword)&startIndex=\(startIndex)&count=\(count)"
        urlPath += urlAddr
        print(urlPath)
